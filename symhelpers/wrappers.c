...
extern void *_sym_read_memory(uint8_t *addr, size_t length, bool little_endian);
extern void _sym_write_memory(uint8_t *addr, size_t length, void* expr,
                              bool little_endian);
...
void helper_sym_store_mem_reg(CPUX86State *env, void *expr, uint64_t reg)
{
    ...
    _sym_write_memory((uint8_t*) &env->regs[reg], 8, expr, true);
}

void *helper_sym_load_mem_reg(CPUX86State *env, uint64_t reg)
{
    // printf("CHECKING EXPR for reg %lu at %p\n", reg, &env->regs[reg]);
    void *expr = _sym_read_memory((uint8_t*) &env->regs[reg], 8, true);
#if 0
    if (expr)
    {
        const char *s_expr = _sym_expr_to_string(expr);
...
#endif
    _sym_write_memory((uint8_t*) &env->regs[reg], 8, NULL, true);
    return expr;
}
